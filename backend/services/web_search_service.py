...
class WebSearchContext:
    ...
    citation_map: Dict[str, int] = field(default_factory=dict)  # citation_id -> citation_number
    ...
    def get_citation_bibliography(self) -> str:
        ...
        sorted_results = sorted(
            self.results,
            key=lambda r: self.citation_map.get(r.citation_id, 999)
        )
...
class WebSearchService:
    ...
    def __init__(
        ...
        self.educational_only = educational_only if educational_only is not None else search_config.educational_only
        self.min_relevance_score = min_relevance_score or search_config.min_relevance_score
        ...
        self._educational_keywords = {
            'teaching', 'education', 'learning', 'curriculum', 'lesson',
            'pedagogy', 'instruction', 'classroom', 'student', 'academic',
            'music education', 'music theory', 'music pedagogy', 'music lesson',
            'musical development', 'music curriculum', 'teaching music'
        }
    ...
    async def search_educational_resources(
        ...
        try:
            ...
            optimized_query = self._build_educational_query(
                query, grade_level, subject
            )
            
            # Execute search
            start_time = time.time()
            raw_results = await self._execute_brave_search(
                optimized_query, max_results
            )
        ...
        for result in results:
            ...
            if include:
                ...
                educational_indicators = {
                    'lesson plan', 'teaching', 'education', 'curriculum',
                    'activity', 'assessment', 'learning objective',
                    'music teacher', 'classroom', 'student'
                }
    ...
    def _score_educational_relevance(
        ...
        for result in results:
            ...
            keyword_matches = sum(
                1 for keyword in self._educational_keywords
