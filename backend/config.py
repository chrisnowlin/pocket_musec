...
class DatabaseConfig:
    ...
    path: str = field(
        default_factory=lambda: os.getenv("DATABASE_PATH", "data/pocket_musec.db")
    )
    timeout: int = 30
...
class ChutesConfig:
    ...
    embedding_base_url: str = field(
        default_factory=lambda: os.getenv(
            "CHUTES_EMBEDDING_BASE_URL",
            "https://chutes-qwen-qwen3-embedding-8b.chutes.ai/v1",
        )
    )
    timeout: int = 120
    max_retries: int = 3
...
class LLMConfig:
    ...
    default_max_tokens: int = field(
        default_factory=lambda: int(os.getenv("DEFAULT_MAX_TOKENS", "2000"))
    )
...
class ImageProcessingConfig:
    ...
    storage_path: str = field(
        default_factory=lambda: os.getenv("IMAGE_STORAGE_PATH", "data/images")
    )
...
class WebSearchConfig:
    ...
    cache_ttl: int = field(
        default_factory=lambda: int(os.getenv("WEB_SEARCH_CACHE_TTL", "3600"))
    )  # 1 hour
    max_cache_size: int = field(
        default_factory=lambda: int(os.getenv("WEB_SEARCH_MAX_CACHE_SIZE", "100"))
    )
    timeout: int = field(
        default_factory=lambda: int(os.getenv("WEB_SEARCH_TIMEOUT", "30"))
    )  # 30 seconds
    educational_only: bool = field(
        default_factory=lambda: os.getenv(
            "WEB_SEARCH_EDUCATIONAL_ONLY", "false"
        ).lower()
        == "true"
    )
    min_relevance_score: float = field(
        default_factory=lambda: float(
            os.getenv("WEB_SEARCH_MIN_RELEVANCE_SCORE", "0.3")
        )
    )
    max_results: int = field(
        default_factory=lambda: int(os.getenv("WEB_SEARCH_MAX_RESULTS", "10"))
    )
...
class FileStorageConfig:
    ...
    storage_root: str = field(
        default_factory=lambda: os.getenv("FILE_STORAGE_ROOT", "data/uploads")
    )
    ...
    allowed_extensions: List[str] = field(
        default_factory=lambda: [
            ...
            for ext in os.getenv(
                "ALLOWED_EXTENSIONS",
                ".pdf,.doc,.docx,.txt,.jpg,.jpeg,.png,.gif,.webp,.tiff,.tif",
            ).split(",")
        ]
    )
    duplicate_detection: bool = field(
        default_factory=lambda: os.getenv("DUPLICATE_DETECTION", "true").lower()
        == "true"
    )
    retention_days: int = field(
        default_factory=lambda: int(os.getenv("FILE_RETENTION_DAYS", "365"))
    )
    cleanup_enabled: bool = field(
        default_factory=lambda: os.getenv("FILE_CLEANUP_ENABLED", "true").lower()
        == "true"
    )
