...
from llm.embeddings import StandardsEmbeddings
from citations.citation_repository import CitationRepository
...
def test_file_linking_integration():
    ...
    try:
        ...
        assert 'FOREIGN KEY (file_id) REFERENCES uploaded_files(file_id)' in table_sql, "Foreign key constraint missing for standards.file_id"
        ...
        assert retrieved_standard.file_id == test_file_id, "file_id not preserved in retrieval"
        ...
        assert standards_by_file[0].file_id == test_file_id, "Incorrect file_id in retrieved standards"
        ...
        embedding_with_file = embeddings_manager.get_objective_embedding_with_file("TEST.OBJ.1.1.1")
        assert embedding_with_file is not None, "Failed to retrieve objective embedding with file_id"
        retrieved_embedding, retrieved_file_id = embedding_with_file
        assert retrieved_file_id == test_file_id, "file_id not correctly retrieved with embedding"
        ...
        assert citations_by_file[0].file_id == test_file_id, "Incorrect file_id in retrieved citations"
