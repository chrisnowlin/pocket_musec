        ...
        for doc_type, patterns in self.FILENAME_PATTERNS.items():
            for pattern in patterns:
                if re.search(pattern, filename_lower):
                    logger.debug(f"Filename matched pattern '{pattern}' -> {doc_type.value}")
                    return doc_type
            ...
            with pdfplumber.open(pdf_path) as pdf:
                ...
                if scores:
                    ...
                    if scores[best_type] > 0:
                        ...
                        logger.info(f"Classified as {best_type.value} (score: {scores[best_type]})")
                        return best_type
    ...
    def classify(self, pdf_path: str) -> Tuple[DocumentType, float]:
        ...
        if filename_type and filename_type != DocumentType.UNKNOWN:
            ...
            if filename_type == content_type:
                ...
                logger.info(f"Document classified as {filename_type.value} (high confidence)")
                return filename_type, 1.0
            elif content_type == DocumentType.UNKNOWN:
                # Content inconclusive, trust filename
                logger.info(f"Document classified as {filename_type.value} (medium confidence - filename only)")
                return filename_type, 0.7
            else:
                # Disagreement - trust content over filename
                logger.warning(f"Filename suggested {filename_type.value} but content suggests {content_type.value}")
            ...
            if content_type != DocumentType.UNKNOWN:
                logger.info(f"Document classified as {content_type.value} (medium confidence - content only)")
                return content_type, 0.6
            else:
