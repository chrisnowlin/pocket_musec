...
class VisionFirstStrategy(ExtractionStrategy):
    ...
    def _extract_with_vision(self, pdf_path: str) -> List[ParsedStandard]:
        """Extract standards using vision model"""
        all_standards = {}
                ...
                try:
                    ...
                    for std in page_results:
                        if std.standard_id not in all_standards:
                            ...
                            existing_obj_ids = self.base_parser._extract_objective_ids(existing.objectives)
                            
                            for obj in std.objectives:
                                obj_id = self.base_parser._extract_objective_id(obj)
        ...
        try:
            ...
            for std_id, std_data in standards_dict.items():
                ...
                parsed_standard = ParsedStandard(
                    ...
                    strand_name=self.base_parser.strand_mappings.get(strand_code, ("Unknown", ""))[0],
                    strand_description=self.base_parser.strand_mappings.get(strand_code, ("", "Unknown"))[1],
                    standard_id=std_id,
        ...
        with pdfplumber.open(pdf_path_obj) as pdf:
            ...
            for page_num, page in enumerate(pages_to_process, 1):
                ...
                for table in tables:
                    ...
                    table_strand_info = self.base_parser._extract_strand_from_table(table)
...
class NCStandardsParser:
    ...
    def parse_standards_document(self, pdf_path: str, **kwargs) -> List[ParsedStandard]:
        ...
        if not strategy_instance.is_available():
            logger.warning(f"Strategy {strategy_to_use.value} not available, falling back")
            strategy_to_use = self._get_fallback_strategy()
            strategy_instance = self.strategies[strategy_to_use]
        
        # Extract standards using the selected strategy
        try:
            self.parsed_standards = strategy_instance.extract_standards(pdf_path, **kwargs)
            logger.info(f"Successfully extracted {len(self.parsed_standards)} standards")
            ...
            for fallback_strategy in self._get_fallback_strategies():
                ...
                if fallback_instance.is_available():
                    try:
                        self.parsed_standards = fallback_instance.extract_standards(pdf_path, **kwargs)
    ...
    def _extract_standards_comprehensive(self, all_blocks: List[Dict], pages: List[ParsedPage]) -> List[ParsedStandard]:
        ...
        for std_id in sorted(standard_blocks.keys()):
            ...
            if current_grade and current_strand:
                ...
                standard = self._build_standard_from_blocks(std_id, blocks, current_grade, current_strand)
                if standard:
    ...
    def _build_standard_from_blocks(self, std_id: str, blocks: List[Dict], grade: str, strand: Tuple[str, str]) -> Optional[ParsedStandard]:
    ...
    def _extract_standards_positional(self, pages: List[ParsedPage]) -> List[ParsedStandard]:
        ...
        for page in pages:
            page_standards = self._parse_page_positional(page, current_grade, current_strand)
            standards.extend(page_standards)
    ...
    def _parse_page_positional(self, page: ParsedPage, current_grade: Optional[str], 
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "parsing_strategy": self.strategy.value if self.strategy != ParsingStrategy.AUTO else "auto_selected"
        }