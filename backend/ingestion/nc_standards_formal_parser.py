...
from .nc_standards_table_parser import NCStandardsParser as TableParser
...
class NCStandardsParser:
    ...
    def __init__(self, use_vision: bool = True):
        ...
        self.vision_available = use_vision and PDF2IMAGE_AVAILABLE and PIL_AVAILABLE
        ...
        # Initialize table-based parser as primary fallback, structured as secondary
    ...
    def parse_standards_document(self, pdf_path: str) -> List[ParsedStandard]:
        ...
        logger.info(
            f"Parser mode: {'vision (slow)' if self.vision_available else 'hybrid (fast)'}"
        )
        ...
        if self.vision_available:
            try:
                ...
                if self._validate_extraction(self.parsed_standards):
                    ...
                    logger.warning(
                        "Vision extraction validation failed, falling back to hybrid"
                    )
                ...
                logger.error(
                    f"Vision extraction failed: {e}, falling back to hybrid parser"
                )
        ...
        try:
            ...
            self.parsed_standards = self.table_parser.parse_standards_document(
                pdf_path, max_page=34
            )
            logger.info(
                f"Table parser extracted {len(self.parsed_standards)} standards"
            )
            ...
            if self._validate_extraction(self.parsed_standards):
                ...
                logger.warning(
                    "Table parser validation failed, falling back to structured parser"
                )
        except Exception as e:
            logger.error(f"Table parser failed: {e}, falling back to structured parser")

        # Secondary fallback to structured parser
        logger.info("Using structured parser (secondary fallback mode)")
                ...
                try:
                    ...
                    for std in page_results:
                        ...
                        if std.standard_id not in all_standards:
                            ...
                            existing_obj_ids = self._extract_objective_ids(
                                existing.objectives
                            )
        ...
        try:
            ...
            for std_id, std_data in standards_dict.items():
                ...
                parsed_standard = ParsedStandard(
                    ...
                    strand_name=self.strand_mappings.get(strand_code, ("Unknown", ""))[
                        0
                    ],
                    strand_description=self.strand_mappings.get(
                        strand_code, ("", "Unknown")
                    )[1],
                    standard_id=std_id,
