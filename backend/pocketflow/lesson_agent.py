...
class LessonAgent(Agent):
    ...
    def _setup_state_handlers(self) -> None:
        ...
        self.add_state_handler("objective_refinement", self._handle_objective_refinement)
    ...
    def _show_welcome_message(self) -> str:
        ...
        response += "\nPlease enter a number (1-" + str(len(grade_levels)) + ") or type 'quit' to exit."
    ...
    def _show_strand_options(self) -> str:
        ...
        for standard in standards:
            if standard.strand_code not in available_strands:
                available_strands[standard.strand_code] = strand_info.get(standard.strand_code, {})
        
        response = (
            f"📚 Grade {grade} - Select a Strand\n\n"
            "Available strands:\n"
        )
    ...
    def _handle_strand_selection(self, message: str) -> str:
        ...
        try:
            ...
            if 1 <= choice <= len(strand_list):
                ...
                self.lesson_requirements['strand_info'] = available_strands[selected_strand]
    ...
    def _handle_standard_selection(self, message: str) -> str:
        ...
        try:
            ...
            if 1 <= choice <= len(standards):
                ...
                objectives = self.standards_repo.get_objectives_for_standard(selected_standard.standard_id)
            ...
            topic = message.strip()
            recommendations = self.standards_repo.recommend_standards_for_topic(topic, grade, limit=5)
            ...
            response += "Please enter a number to select a standard or try a different topic."
