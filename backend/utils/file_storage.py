...
class FileStorageManager:
    ...
    def __init__(self, storage_root: Optional[str] = None):
        ...
        """
        self.storage_root = Path(storage_root or config.file_storage.storage_root).resolve()
    ...
    def save_file_permanently(self, temp_file_path: str, original_filename: str) -> Tuple[str, str]:
        ...
        try:
            ...
            if not self.is_allowed_extension(original_filename):
                raise ValueError(f"File extension not allowed: {self.get_file_extension(original_filename)}")
    ...
    def cleanup_old_files(self, retention_days: Optional[int] = None, batch_size: int = 100) -> Dict[str, Any]:
        ...
        try:
            ...
            while True:
                try:
                    with file_repo.db_manager.get_connection() as conn:
                        ...
                        offset += batch_size
                        logger.info(f"Processed batch of {len(batch_files)} files (total: {deleted_count} deleted)")
            ...
            logger.info(f"Cleanup completed: deleted {deleted_count} files, freed {freed_bytes} bytes, {error_count} errors")
        ...
        try:
            ...
            for file_path in self.storage_root.rglob("*"):
                if file_path.is_file():
                    ...
                    file_counts_by_extension[extension] = file_counts_by_extension.get(extension, 0) + 1
